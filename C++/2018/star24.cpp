...
int main() {
	int start_pos=0, end_pos=0, add_count, previous_difference, difference;
	int generation = 1, pot_plants = 0, stable_count;
	long value=0, previous_value = 0;
	...
	std::string hundreds;
	int buffer;
	bool new_state = false, rule_app=false, unstable=true;
	...
	while(unstable) {
		...
		std::cout << value << ":" << difference << std::endl;
	...
	value += difference*(50000000000-generation+1);

	std::cout << "At generation fifty billion, the value is: " << value << std::endl;
	std::cout << std::endl;
}
