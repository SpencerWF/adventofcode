...
int main() {
	...
	for(second_count=0; stop!="y"; second_count++) {
		adjacent_count = 0;
		for(int j=0; j<stars.size(); j++) {
		...
		if(adjacent_count > stars.size()*0.5) {
			...
			std::cout << "Type a letter and press enter to continue, type y if you want to end the program." << std::endl;
			std::cin >> stop;
		}
	}
	std::cout << std::endl;
