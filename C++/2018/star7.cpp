...
int main() {
	...
	for(int i=0; i<minutes_list.size(); i++) {
		std::cout << "Guard slept in these minutes: " << i << " this many times: " << minutes_list[i] << std::endl;
		if(minutes_list[i]>minutes_list[minute]) {
			minute = i;
		}
	}

	std::cout << "Sleepiest guard is: " << current_ID << std::endl;
	std::cout << "Who slept the most during minute " << minute << std::endl;
	std::cout << "The value is: " << current_ID * (minute) << std::endl;
}
